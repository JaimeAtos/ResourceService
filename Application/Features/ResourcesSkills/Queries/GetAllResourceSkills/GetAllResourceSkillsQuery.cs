...
public class GetAllResourceSkillsQueryHandler : IRequestHandler<GetAllResourceSkillsQuery, PagedResponse<List<ResourceSkillsDto>>>
{
    ...
    {
        var pagination = new PagedResourcesSkillsSpecification(request.PageSize, request.PageNumber, request.ResourceName, request.SkillName,
            request.SkillAcceptanceURL, request.IsComplice);
