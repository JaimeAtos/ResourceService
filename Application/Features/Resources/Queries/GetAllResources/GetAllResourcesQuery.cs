...
public class GetAllResourcesQueryHandler : IRequestHandler<GetAllResourcesQuery, PagedResponse<List<ResourceDto>>>
{
    ...
    {
        var pagination = new PagedResourcesSpecification(request.PageSize, request.PageNumber, request.FullName, request.WorkEmail, request.Phone, request.CurrentStateDescription,
            request.CurrentPositionDescription,request.NessieID,request.CurrentClientName);
